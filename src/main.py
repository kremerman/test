...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Vergelijk koop (user cost + cashflow) met markthuur in huidige prijzen."
    )
    ...
    parser.add_argument("--annual-inflation-rate", type=float, default=0.02)
    parser.add_argument("--annual-discount-rate", type=float, default=0.03)
    ...
    parser.add_argument("--annual-imputed-rent-rate", type=float, default=0.0035)
    parser.add_argument("--mortgage-interest-deductible-fraction", type=float, default=1.0)
    parser.add_argument("--mortgage-interest-tax-rate", type=float, default=0.37)
    parser.add_argument("--wet-hillen-fraction", type=float, default=0.0)
...
def main() -> None:
    ...
    inputs = HousingCostInput(
        ...
        annual_imputed_rent_rate=args.annual_imputed_rent_rate,
        mortgage_interest_deductible_fraction=args.mortgage_interest_deductible_fraction,
        mortgage_interest_tax_rate=args.mortgage_interest_tax_rate,
        wet_hillen_fraction=args.wet_hillen_fraction,
    ...
    print("Alle bedragen zijn in huidige prijzen (reëel).")
    print("Jaar | User cost/mnd | Cashflow/mnd | Markthuur/mnd | Koop-huur")
    for row in rows[:5]:
        print(
            f"{row.year:>4} | € {row.monthly_user_cost_real:>10.2f} | "
            f"€ {row.monthly_cashflow_cost_real:>10.2f} | "
            f"€ {row.monthly_market_rent_cost_real:>10.2f} | "
            f"€ {row.monthly_owner_user_cost_minus_renter_market_cost_real:>10.2f}"
        )
